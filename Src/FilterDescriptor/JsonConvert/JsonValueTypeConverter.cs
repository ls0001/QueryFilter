...
/// Json元素值类型处理类（线程安全）
/// </summary>
public sealed class JsonValueTypeConverter  
{
    ...
    internal static object? ConvertJsonElement(JsonElement element, Type targetType, JsonValueConvertOptions options)
    {
    ...
    private static object ParseString(string value, Type targetType, JsonValueConvertOptions options)
    {
        return targetType switch
        {
            ...
            _ when targetType.IsEnum => ParseEnum(value, targetType),
        ...
        static object ParseEnum(string value, Type enumType)
        {
    ...
    {
        // 尝试解析为Unix时间戳
        if ( long.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture, out var number) )
        {
            return options.UnixTimestampUnit == UnixTimestampUnit.Milliseconds
